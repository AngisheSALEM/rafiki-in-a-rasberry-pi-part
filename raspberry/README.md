...
Le client envoie un heartbeat toutes les cinq secondes. Le PC affiche le corps
comme connecté uniquement s'il reçoit ce heartbeat récent. Si le PC affiche
« En attente de la Raspberry », vérifie que `IP_DU_PC` est bien l'adresse du
PC sur le Wi-Fi, jamais `127.0.0.1` depuis la Raspberry.
